...
output "ssh_user" {
  ...
  value       = aws_instance.ec2_ssh_server.user_data
}

resource "local_file" "fingerprint" {
  content  = aws_key_pair.tf_key.fingerprint
  filename = "${path.module}/.ssh/fingerprint"
}
...
resource "local_file" "hosts" {
  ...
  [circleci]
  localhost ansible_connection=local ansible_virtualization_type=docker ansible_python_interpreter=/usr/bin/python3
  EOF
